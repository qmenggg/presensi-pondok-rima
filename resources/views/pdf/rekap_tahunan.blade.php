...
@section('content')
    ...
    <div class="header-title">LAPORAN REKAPITULASI PRESENSI TAHUNAN</div>
    <div class="header-subtitle">
        Tahun Pelajaran: {{ $tapel->nama_tapel }}<br>
        Periode: {{ $tapel->tanggal_mulai->locale('id')->isoFormat('D MMMM Y') }} - {{ $tapel->tanggal_selesai->locale('id')->isoFormat('D MMMM Y') }}
    </div>
            ...
            @if(count($santriStats) === 0)
                <tr>
                    <td colspan="9" style="text-align: center; padding: 20px;">
                        Tidak ada data presensi pada tahun pelajaran ini.
                    </td>
                </tr>
            @endif
        </tbody>
